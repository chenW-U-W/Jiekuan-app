...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        touchesArray = [[NSMutableArray alloc]initWithCapacity:0];//保存得是被选中按钮的坐标点
        touchedArray = [[NSMutableArray alloc]initWithCapacity:0];//保存的是被选中的第一个btn
    ...
    if (touch) {
        ...
        for (int i=0; i<buttonArray.count; i++) {
            ...
            if (CGRectContainsPoint(buttonTemp.frame,touchPoint)) {
                ...
                [touchesArray addObject:dict];//touchesArray 顺序保存的是被选中的按钮
                break;
            }
        }
        lineEndPoint = touchPoint;
